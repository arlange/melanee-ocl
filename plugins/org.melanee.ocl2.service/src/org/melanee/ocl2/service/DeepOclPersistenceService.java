...
public class DeepOclPersistenceService {
	 ...
	 * each line and checking at the beginning if a constraint type is
	 * mentioned, like inv for invariant.
	 */
		...
		for (int i = 0; i < parts.length; i++) {
			if (parts[i].trim().startsWith("inv")) {
				...
				String name = constraintString
						.substring(constraintString.indexOf("inv") + "inv".length(), constraintString.indexOf(":"))
						.trim();
			...
			if (parts[i].trim().startsWith("derive")) {
				...
				for (AbstractConstraint constraint : this.context.getConstraint()) {
					...
					if (c.getClass().getSimpleName().equals("DeriveConstraintImpl") && c.getName().equals("derive")) {
			...
			if (parts[i].trim().startsWith("body")) {
				for (AbstractConstraint constraint : this.context.getConstraint()) {
					Constraint c = (Constraint) constraint;
					if (c.getClass().getSimpleName().equals("BodyConstraintImpl") && c.getName().equals("body")) {
						c.getExpression().clear();
						this.constraint = c;
					} else {
			...
			if (parts[i].trim().startsWith("post")) {
				for (AbstractConstraint constraint : this.context.getConstraint()) {
					Constraint c = (Constraint) constraint;
					if (c.getClass().getSimpleName().equals("PostConstraintImpl") && c.getName().equals("post")) {
						c.getExpression().clear();
					} else {
			...
			if (parts[i].trim().startsWith("pre")) {
				for (AbstractConstraint constraint : this.context.getConstraint()) {
					Constraint c = (Constraint) constraint;
					if (c.getClass().getSimpleName().equals("PresConstraintImpl") && c.getName().equals("pre")) {
						c.getExpression().clear();
					} else {
	...
	/**
	 * this method updates all context relevant information when a navigation in
	 * the model happens
	 * 
	 * @param context
	 */
	...
	/**
	 * this method removes a constraint identified by its type and its name from
	 * the clabject
	 * 
	 * @param kind
	 * @param constraint
	...
	public void removeConstraint(String constraint) {
		...
		removeCommand = RemoveCommand.create(editingDomain, this.context, PLMPackage.eINSTANCE.getElement_Constraint(),
				c);
