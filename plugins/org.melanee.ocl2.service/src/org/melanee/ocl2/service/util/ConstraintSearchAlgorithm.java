...
public class ConstraintSearchAlgorithm {
	 ...
	 * @return The found visualizer; null if no visualizer is found; A copy of
	 *         the visualizer with aspects merged into it if aspects are found
	 *         and mergeAspects == true
	 */
	private List<AbstractConstraint> search(Level e, List<Class> constraintTypes) {
		...
		for (int i = deepModel.getContent().indexOf(e) - 1; i > 0; i--) {
			constraints.addAll(getConstraintFromElement((Element) deepModel.getContent().get(i), e, constraintTypes));
		}
		return constraints;
	}
	 ...
	 * visualizer are stored and merged into the visualizer which is used for
	 * rendering (mergeAspects == true). In the case of a level the level itself
	 * is searched for a visualizer, then the levels into the more absract
	 * direction. For levels no aspects are supported.
	 * 
	 * @param e
	...
	/**
	 * Searches a clabject for a constraint. The constraint has to have
	 * the correct notation, format and conform to the isSameLevel attribute.
	 * 
	 * @param currentClabject
	 *            The clabject to search the constraint for
	 * @param elementToValidate
	 *            The element to validate. This is needed to check if the
	 *            element to validate and the constraint are at the same level.
	 * @return Returns a List<AbstractConstraint>.
	 */
	private List<AbstractConstraint> getConstraintFromElement(Element currentElement, Element elementToValidate,
	...
	private boolean isApplicable(AbstractConstraint constraint, Element elementToValidate,
			List<Class> constraintTypes) {
		return constraintTypes.contains(constraint.getClass()) && DeepOCL2Util.isInLevel(elementToValidate, constraint);
	}
