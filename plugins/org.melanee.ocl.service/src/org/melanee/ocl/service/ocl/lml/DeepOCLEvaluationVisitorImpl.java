 ...
 *
 */
public class DeepOCLEvaluationVisitorImpl
		extends
	 ...
	 * @param evalEnv An EvaluationEnvironment
	 * @param extentMap An ExtentMap
	 */
	...
	/* (non-Javadoc)
	 * @see org.eclipse.ocl.AbstractEvaluationVisitor#getOperationBody(java.lang.Object)
	 */
	...
	protected OCLExpression<EObject> getOperationBody(EObject operation) {
		if (operation instanceof EOperation) {
			...
			if (result == InvocationBehavior.NO_OCL_DEFINITION) {
				...
				String expr = OCLCommon.getDelegateAnnotation(
						(EModelElement) operation, "body");
				if (expr != null) {
					EClass context = ((EOperation) operation)
							.getEContainingClass();
					OCL<EPackage,EObject,EObject,EObject,Enumeration,EObject,EObject,CallOperationAction,SendSignalAction,Constraint,EObject,EObject> ocl = OCL.newInstance(getEnvironment().getFactory());
					...
					try {
						constraint = (Constraint) helper
								.createBodyCondition(expr);
					} catch (ParserException e) {
						throw new org.eclipse.ocl.ecore.delegate.OCLDelegateException(
					...
					OCLExpression<EObject> body = (OCLExpression<EObject>) specification
							.getBodyExpression();
	...
	/* (non-Javadoc)
	 * @see org.eclipse.ocl.EvaluationVisitorImpl#visitIteratorExp(org.eclipse.ocl.expressions.IteratorExp)
	 */
	public Object visitIteratorExp(IteratorExp<EObject, EObject> ie) {
		...
		if (ie.getName() == "collect") {
			if (ie.getBody() instanceof PropertyCallExp) { // cast destination
				@SuppressWarnings("rawtypes")
				PropertyCallExp propertyCallExp =  (PropertyCallExp) (ie
						.getBody());
				if (propertyCallExp.getReferredProperty() instanceof NavigationAttribute) {
					...
					((DeepOCLEvaluationEnvironment) getEvaluationEnvironment())
							.setLastCast(newCast);
				} else if (propertyCallExp.getReferredProperty() instanceof LevelCastAttribute) {
					LevelCastAttribute levelCastAttribute = (LevelCastAttribute) (propertyCallExp.getReferredProperty());
					CastData newCast = new CastData();
	...
	/* (non-Javadoc)
	 * @see org.eclipse.ocl.EvaluationVisitorImpl#visitPropertyCallExp(org.eclipse.ocl.expressions.PropertyCallExp)
	 */
	public Object visitPropertyCallExp(PropertyCallExp<EObject, EObject> pc) {
		...
		if (pc.getReferredProperty() instanceof NavigationAttribute
				&& !(pc.getSource() instanceof VariableExp)) { // only cast if
																// source is not
																// a
																// variable, in
																// normal case
																...
																// iteration has
																// finished
			NavigationAttribute navAttribute = ((NavigationAttribute) pc
					.getReferredProperty());
			CastData newCast = new CastData();
			newCast.setDestination(navAttribute.getNavigationValue());
			newCast.setSource(null);
		...
		if (pc.getReferredProperty() instanceof LevelCastAttribute) {
			...
			if ((sourceVal!=levelCastAttribute.getCast())&&!(DeepOCLUtilities.getAllTypes((Clabject) sourceVal).contains(levelCastAttribute.getCast()))) { 
				return getInvalid();
			}
	...
	/* (non-Javadoc)
	 * @see org.eclipse.ocl.EvaluationVisitorImpl#visitOperationCallExp(org.eclipse.ocl.expressions.OperationCallExp)
	 */
		...
		if (sourceVal instanceof Clabject) {
			...
			if (opCode == PredefinedType.ALL_INSTANCES && args.size() == 2) {
				...
				if (args.get(0) instanceof IntegerLiteralExp
						&& args.get(1) instanceof IntegerLiteralExp) {
					int distance1 = ((IntegerLiteralExp) args.get(0))
							.getIntegerSymbol();
					int distance2 = ((IntegerLiteralExp) args.get(1))
							.getIntegerSymbol();
					if (distance1 >= 1 && distance2 > distance1) {
						...
						while (tempDist <= distance2) {
							allInstances.addAll(DeepOCLUtilities
									.getInstancesWithDistance(sourceClabject,
											tempDist));
							tempDist++;
						}
						return allInstances;
			...
			if (opCode == PredefinedType.ALL_INSTANCES && args.size() == 1) {
				...
				Clabject sourceClabject = (Clabject) sourceVal;
				if (args.get(0) instanceof IntegerLiteralExp) {
					int distance = ((IntegerLiteralExp) args.get(0))
							.getIntegerSymbol();
					if (distance >= 1) {
						return DeepOCLUtilities.getInstancesWithDistance(
								sourceClabject, distance);
					}
				}
