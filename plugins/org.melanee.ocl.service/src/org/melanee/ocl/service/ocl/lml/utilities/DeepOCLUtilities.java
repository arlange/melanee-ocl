...
public final class DeepOCLUtilities {
	 ...
	 */
	public static HashSet<Clabject> getAllParticipantsForName(
			Clabject clabject, String name) {
		...
		if (clabject instanceof Connection) {
			...
			for (ConnectionEnd connectionEnd : connection
					.getAllConnectionEnd()) {
				if (connectionEnd.getMoniker() != null
						&& connectionEnd.getMoniker().equals(name) || (connectionEnd.getDestination() != null && connectionEnd
								.getDestination().getName().equals(name))) {
		...
		for (Connection connection : clabject.getConnections()) {
			...
			for (ConnectionEnd connectionEnd : connection
					.getAllConnectionEnd()) {
				if ((connectionEnd.getMoniker() != null && connectionEnd
						.getMoniker().equals(name))
						|| (connectionEnd.getDestination() != null && connectionEnd.getDestination().getName() !=null&& connectionEnd
								.getDestination().getName().equals(name))) {
					if (connectionEnd.getDestination() != clabject)
						participants.add(connectionEnd.getDestination());
				}
	 ...
	 * @param navAttribute
	 * @param source
	 * @return A ArrayList of participant of a NavigationAttribute from an instanc viepoint
	 */
	public static ArrayList<Clabject> getAllSubtypeParticpants(
			NavigationAttribute navAttribute, Clabject source) {
			...
			for (Connection superTypeConnection : superTypeConnections) {
				for (Object instance : getAllInstances(superTypeConnection)) {
					...
					if (connectionInstance.getParticipants().contains(source)) {
						for (Clabject participant : connectionInstance
								.getParticipants()) {
							if (participant != source
								...
								for (Object aType : allTypes) {
									if (superTypeParticants.contains(aType)) {
										//check multiplicit pf connectionEnd so that for multiple connectionEnds the particpants are inserted multiple times
										ConnectionEnd connectionEnd = getConnectionEndForParticipant(connectionInstance, participant); 
										if (connectionEnd!=null && connectionEnd.getUpper() > 1) {
											int participantAmount = connectionEnd
													.getUpper();
											int count = 0;
	 ...
	 */
	public static List<Clabject> getSuperTypeParticipants(
			NavigationAttribute navAttribute) {
		ArrayList<Clabject> superTypeParticipants = new ArrayList<Clabject>();
		List<Connection> superTypeConnections = getSuperTypeConnections(navAttribute);
		for (Connection superTypeConnection : superTypeConnections) {
			List<ConnectionEnd> allConnectionEnds = superTypeConnection
					.getConnectionEnd();
			for (ConnectionEnd connectionEnd : allConnectionEnds) {
				if ((connectionEnd.getMoniker() != null && connectionEnd
						.getMoniker().equals(navAttribute.getName()))
						|| (connectionEnd.getDestination() != null && connectionEnd
								.getDestination().getName()
								.equals(navAttribute.getName()))) {
	 ...
	 */
	public static List<Connection> getSuperTypeConnections(
			NavigationAttribute navAttribute) {
		...
		for (Connection clabjectConnection : clabjectConnections) {
			...
			if (navAttribute.getConnection() != null) {
				if (clabjectConnection.getMoniker().contains(
						navAttribute.getName())
				...
				if (clabjectConnection.getName() != null
						&& clabjectConnection.getName().equals(
								navAttribute.getName())) {
					superTypeConnection.add(clabjectConnection);
				}
			}
	...
	public static List<Object> getAllDirectTypes(Clabject clabject) {
		...
		ArrayList<Object> directTypes = new ArrayList<Object>(
				Arrays.asList(directTypesArray));
	 ...
	 * @param distance
	 * @return A List of (deep) instance with the given level instance from the sourceClabject
	 */
	@SuppressWarnings("unchecked")
	public static List<Clabject> getInstancesWithDistance(
			Clabject sourceClabject, int distance) {
	 ...
	 */
	private static ConnectionEnd getConnectionEndForParticipant(
			Connection connection, Clabject participant) {
